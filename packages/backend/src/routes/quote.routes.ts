...
const BUILT_IN_QUOTES = [
  ...
  { text: '行动是治愈恐惧的良药，而犹豫拖延将不断滋养恐惧。', author: '戴尔·卡耐基' },
  { text: '少壮不努力，老大徒伤悲。', author: '汉乐府' },
  { text: '专注做一件事，把它做到极致。', author: '佚名' },
  { text: '学习永远不会晚。', author: '乔治·艾略特' },
  ...
  { text: '成功的秘诀在于坚持不懈。', author: '佚名' },
  { text: '种一棵树最好的时间是十年前，其次是现在。', author: '非洲谚语' },
  { text: '自律的人生才自由。', author: '佚名' },
...
quoteRouter.get('/random', (_req, res) => {
  const index = Math.floor(Math.random() * BUILT_IN_QUOTES.length);
  res.json(BUILT_IN_QUOTES[index]);
});

// GET /api/quotes/all
quoteRouter.get('/all', (_req, res) => {
  res.json(BUILT_IN_QUOTES);
});
